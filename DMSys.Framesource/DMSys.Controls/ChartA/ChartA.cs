...
namespace DMSys.Controls.ChartA
{
    ...
    /// <summary>
    /// Represents a basic time series charting control.F
    /// </summary>
    public partial class ChartA : Control
    {
        #region Consts
        ...
        #region Fields
        private BackgroundImageStyle backgroundImageStyle = ChartA.defaultBackgroundImageStyle;
        private ChartTitleSettings title = null;
        private ChartValueAxisSettings xAxis = null;
        ...
        public ChartA()
        {
            ...
            this.xAxis = new ChartValueAxisSettings(this);
            ...
            // initialize members
            base.BackColor = ChartA.defaultBackColor;
            base.ForeColor = ChartA.defaultForeColor;
        }
        ...
        /// </summary>
        [Category("Appearance")]
        [DefaultValue(ChartA.defaultBackgroundImageStyle)]
        ...
        {
            return (base.BackColor != ChartA.defaultBackColor);
        }
        ...
        {
            return (base.ForeColor != ChartA.defaultForeColor);
        }
        ...
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
        public ChartValueAxisSettings XAxis
        {
            get
            {
                return this.xAxis;
        ...
        private Point _PositionMouse = Point.Empty; // Позиция на мишката
        private bool _IsMouseDown = false;          // Индикатор за натиснат бутон на мишката
        private ChartSeriesValue _SeriesValue = null;   // 
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            if (this.legend.Visible)
            {
                ...
                legendWidth += ChartA.LEGEND_MARGIN * 4;
                legendWidth += ChartA.MARK_SIZE * 4;
                legendWidth += itemSize.Width;
                legendHeight += ChartA.LEGEND_MARGIN * 2;
            ...
            // Рисува позицията на мишката в координати на графиката
            this.OnDrawMousePosition(new DrawChartEventArgs(pe.Graphics, me));
            this.DoChangeMarkPosition();            
        }
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            _PositionMouse.X = e.X;
            _PositionMouse.Y = e.Y;
        ...
        protected virtual void DoChangeMarkPosition()
        {
            // check if there are subscribers
            if ( (this.ChangeMarkPosition != null) && (_SeriesValue!=null) )
            {
                this.ChangeMarkPosition(this, _SeriesValue);
            }
        }
        ...
        /// </summary>
        /// <param name="e"></param>
        protected virtual void OnDrawMousePosition(DrawChartEventArgs e)
        {
            // Ако има натиснат бутон на мишката
            if (_IsMouseDown)
            {
                ...
                if (e.Cancel == false)
                {
                    ...
                    try
                    {
                        ...
                        int xMin = this.xAxis.Scale.Minimum;
                        int xMax = this.xAxis.Scale.Maximum;
                        ...
                        decimal MSValueY = Convert.ToDecimal(((((double)(start.Y - _PositionMouse.Y)) / yLen) * (yMax - yMin)) + yMin);
                        decimal MSValueX = Convert.ToDecimal(((double)(_PositionMouse.X - start.X) / xLen) * (xMax - xMin)) + xMin;

                        if (_SeriesValue != null)
                        {
                            // Променя по X
                            if (!XAxis.Scale.LockPosition)
                                _SeriesValue.ValueX = MSValueX;
                            // Променя по Y
                            if (!YAxis.Scale.LockPosition)
                                _SeriesValue.ValueY = MSValueY;
                        }

                        e.Graphics.DrawString(Math.Round(MSValueX,3).ToString() + " - " + Math.Round(MSValueY,3).ToString(), this.xAxis.Labels.Font, textBrush, _PositionMouse.X + Cursor.HotSpot.X, _PositionMouse.Y);
                    }
                    finally
                    {
        ...
        protected virtual void OnDrawChartLegend(DrawChartEventArgs e)
        {
            ...
            if (e.Cancel == false && this.legend.Visible)
            {
                ...
                try
                {
                    ...
                    lineCap.Offset(ChartA.LEGEND_MARGIN, ChartA.LEGEND_MARGIN);
                    lineCap.Offset(0, Math.Max(itemSize.Height / 2, ChartA.MARK_SIZE));

                    Point lineCap2 = lineCap;
                    lineCap2.Offset(ChartA.MARK_SIZE * 4, 0);

                    // draw sample series line & mark
                    e.Graphics.DrawLine(linePen, lineCap, lineCap2);
                    lineCap.Offset(ChartA.MARK_SIZE * 2, 0);
                    this.DoDrawChartMark(e.Graphics, markBorderPen, markFillPen, lineCap, this.series.Mark.Shape, ChartA.MARK_SIZE);
                    lineCap2.Offset(ChartA.LEGEND_MARGIN, 0);

                    // draw series title
                    Rectangle itemRect = new Rectangle(lineCap2, new Size(itemSize.Width + ChartA.LEGEND_MARGIN * 2, 0));
        ...
        protected virtual void OnDrawChartXAxis(DrawChartEventArgs e)
        {
            ...
            if (e.Cancel == false && this.xAxis.Line.Visible && this.xAxis.Line.Weight > 0)
            {
                ...
                try
                {
                    ...
                    int min = this.xAxis.Scale.Minimum;
                    int max = this.xAxis.Scale.Maximum;
                    ...
                    if (step > 0 && max > min)
                    {
                        ...
                        for (int tick = min; tick <= max; tick += step)
                        {
                            ...
                            int tickDate = min + tick;
                            string tickLabel = tickDate.ToString();
                            Point tickCap = start;
                    ...
                    if (xMax > xMin && yMax > yMin)
                    {
                        ...
                        try
                        {
                            ...
                            for (int bubble = 0; bubble < count; bubble++)
                            {
                                ...
                                {
                                    if (sortedValues[bubble].ValueX < sortedValues[lookup].ValueX)
                                    {
                            ...
                            {
                                ChartSeriesValue seriesValue = sortedValues[i];

                                double xProc = (double)(seriesValue.ValueX - xMin) / (xMax - xMin);
                                int xValue = start.X + (int)Math.Floor(xProc * xLen);

                                // determine position on value scale
                                double yProc = ((double)seriesValue.ValueY - yMin) / (yMax - yMin);
                                ...
                                if (_IsMouseDown)
                                {
                                    if (((_PositionMouse.X - 2) <= crt.X) && ((_PositionMouse.X + 2) >= crt.X)
                                      && ((_PositionMouse.Y - 2) <= crt.Y) && ((_PositionMouse.Y + 2) >= crt.Y))
                                    {
                                        Rectangle circle1 = new Rectangle(crt.X - ChartA.MARK_SIZE - 1,
                                                     crt.Y - ChartA.MARK_SIZE - 1,
                                                     (ChartA.MARK_SIZE * 2) + 2,
                                                     (ChartA.MARK_SIZE * 2) + 2);
                                        e.Graphics.DrawEllipse(borderPen, circle1);
                                        _SeriesValue = seriesValue;
                                    }
                                }

                                // store value for use in the next loop
                                dateLabel = seriesValue.ValueX.ToString();
                                valueLabel = seriesValue.ValueY.ToString();
                                prev = crt;
                            }
        ...
        private Size GetLegendItemSize(Graphics g)
        {
            ...
            // determine the legend item's width
            int itemHeight = Math.Max(ChartA.MARK_SIZE, (int)Math.Ceiling(stringSize.Height));
        ...
        [Category("Drawing")]
        [Description("")]
        public event ChangeMarkPositionEventHandler ChangeMarkPosition;
        /// <summary>
    ...
    #region DrawChart event declarations
        ...
        /// <param name="e"></param>
        public delegate void ChangeMarkPositionEventHandler(object sender, ChartSeriesValue e);
