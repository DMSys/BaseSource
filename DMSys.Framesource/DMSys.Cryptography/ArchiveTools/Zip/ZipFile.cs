...
using DMSys.Cryptography.Checksums;
using DMSys.Cryptography.Zip.Compression.Streams;
using DMSys.Cryptography.Zip.Compression;
using DMSys.Cryptography.Encryption;

namespace DMSys.Cryptography.Zip 
{
	...
	public class ZipFile : IEnumerable
	{
		...
		/// The file ends prematurely
		/// </exception>
		int ReadLeShort(Stream sourceStream)
		{
			return sourceStream.ReadByte() | sourceStream.ReadByte() << 8;
		}
		...
		/// The file ends prematurely
		/// </exception>
		int ReadLeInt(Stream sourceStream)
		{
			return ReadLeShort(sourceStream) | ReadLeShort(sourceStream) << 16;
		}
...
*/
			...
			for (int i = 0; i < entriesForThisDisk; i++) {
				...
				//CJ: All files read here are visible
