...
namespace DMSys.Net.FTP
{
    ...
    public class FTPFileInfo
    {
        ...
        public FTPFileInfo(string line, string path)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    _fileDateTime = FTP_ParseDate(m.Groups["timestamp"].Value);
                }
                catch (Exception)
                {
        ...
        private DateTime FTP_ParseDate(string strValue)
        {
            DateTime FileDate;

            if (strValue[9] == ':')
            {
                FileDate = Convert.ToDateTime(strValue.Substring(0, 3) + "." +
                                               strValue.Substring(4, 2) + "." +
                                               DateTime.Now.Year.ToString() + " " +
                                               strValue.Substring(7, 5) + ":00");
            }
            else
            {
                FileDate = Convert.ToDateTime(strValue.Substring(0, 3) + "." +
                                               strValue.Substring(4, 2) + "." +
                                               strValue.Substring(8, 4) + " 00:00:00");
            }
            return FileDate;
        }
